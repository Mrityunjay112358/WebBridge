...
class WebBridgeAccessibility {
  ...
  highlightIssues(report) {
    Object.entries(report).forEach(([check, selectors]) => {
      selectors.forEach(selector => {
        ...
        if (!node) {
          console.warn(`Could not find element matching selector: ${selector}`);
          return;
        }
  ...
  // Generate accessibility profile from user description
  async generateAccessibilityProfile(userDescription) {
    const systemPrompt = `You are an accessibility assessment AI. Based on the user's description of their needs, challenges, or disabilities, determine which accessibility features they would benefit from.

Return ONLY a valid JSON object with these exact keys and boolean values:
- visual_impairment: true if user mentions blindness, low vision, color blindness, or difficulty seeing
- motor_difficulty: true if user mentions mobility issues, tremors, limited dexterity, or difficulty using mouse/keyboard
- cognitive_difficulty: true if user mentions memory issues, attention problems, learning disabilities, or difficulty processing complex information
- dyslexia: true if user specifically mentions dyslexia or difficulty reading text
- seizure_sensitivity: true if user mentions epilepsy, seizures, or sensitivity to flashing lights/animations
- hearing_impairment: true if user mentions deafness, hearing loss, or difficulty with audio content
- language_barrier: true if user mentions English as second language, translation needs, or difficulty understanding complex text
...
Example response format:
{
  ...
  "language_barrier": false
    ...
    const prompt = `User description: "${userDescription}"

Analyze this description and return the appropriate accessibility profile as JSON.`;

    try {
      const response = await this.callTogetherAI(prompt, systemPrompt);
      if (response) {
      ...
      console.error('Error generating accessibility profile:', error);
    }
    ...
    try {
      ...
      if (simplifiedText) {
        ...
        element.innerHTML = `<div class="webbridge-simplified">${simplifiedText}</div>
          <button class="webbridge-restore-button" onclick="this.parentElement.innerHTML = this.parentElement.getAttribute('data-original-text')">
            Restore Original
          </button>`;
      }
    } catch (error) {
